...
import torchvision
from .adversarial import fgsm_image

if torchvision.__version__ >= "0.3":
    from torchvision.models.utils import load_state_dict_from_url
else:
    import subprocess as sp


    def load_state_dict_from_url(url, progress=True, chunk_size=1024 * 1024):
        bn = os.path.basename(url)
        if not os.path.exists(bn):
            sp.run(["wget", url, "-O", bn])
        model = torch.load(bn)
        return model
...
# Code based on Thomas transfer learning notebook
def train_model(model, dataloaders, dataset_sizes, criterion, optimizer, device, num_epochs=25, return_best=False,
                adversarial=True):
    """
    ...
    for epoch in epochbar:
        ...
        for phase in ['train', 'val']:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    if phase == 'train':
                        if adversarial:
                            ...
                            EPSILON = 0.3
                            adversarial_examples = fgsm_image(inputs, labels, EPSILON, model, criterion, device=device)
                            adversarial_loss = criterion(model(adversarial_examples), labels)
                            # like in goodfellow 2015 paper
                            loss = loss * ADVERSARIAL_ALPHA + (1 - ADVERSARIAL_ALPHA) * adversarial_loss
            ...
            epochbar.write('{} Loss: {:.4f} Acc: {:.4f}'.format(
